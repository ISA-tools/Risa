...
<<Risa-readISAtab>>=
path <- find.package("faahKO")
faahkoISA <- readISAtab(path)
@
...
write.table(maf_character, file=paste(path,"faahko_maf2.csv"),
...
write.assay.file(faahkoISA, assay.filename, path=path)
@
