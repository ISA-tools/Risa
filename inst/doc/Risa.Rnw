...
If you have your own ISA archive, you can use the function \Rfunction{isatab2bioc} to convert it into an R object.
The arguments for the function \Rfunction{isatab2bioc} are:
\begin{itemize}
...
As an example, we can use the \emph{faahKO} dataset, whose latest version contains an ISA dataset describing the experiment. 
...
Additionally, the ISA dataset could be compressed in a .zip file. If that is the case, the function \Rfunction{isatab2bioczip} can be used instead. The only condition is that the ISA-Tab files are contained directly into the zip file, i.e. not inside additional folders.

The parameters for the \Rfunction{isatab2bioczip} function are:
\begin{itemize}
...
The function \Rfunction{addAssayMetadata} allows to modify the metadata in a particular assay file.
The arguments are:
\begin{itemize}
\item{isa}{ An isatab object, as retrieved by the isatab2bioc function.}
...
\begin{itemize}
\item isa: an ISA object, as retrieved by the functions \Rfunction{isatab2bioc} and \Rfunction{isatab2bioczip}
